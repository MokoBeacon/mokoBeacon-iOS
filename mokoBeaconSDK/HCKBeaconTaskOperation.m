...
NSString *const HCKBeaconAdditionalInformation = @"HCKBeaconAdditionalInformation";
NSString *const HCKBeaconDataInformation = @"HCKBeaconDataInformation";
NSString *const HCKBeaconDataStatusLev = @"HCKBeaconDataStatusLev";

@interface HCKBeaconTaskOperation()
...
/**
 线程ID
 */
@property (nonatomic, assign)HCKBeaconTaskOperationID operationID;
...
@implementation HCKBeaconTaskOperation
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error{
    [self parseDataSuccess:[HCKBeaconDataParser parseReadDataFromCharacteristic:characteristic]];
}

- (void)peripheral:(CBPeripheral *)peripheral didWriteValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error{
    [self parseDataSuccess:[HCKBeaconDataParser parseWriteDataFromCharacteristic:characteristic]];
}
    ...
    if (self.completeBlock) {
        if (self.needPartOfData) {
            ...
            NSDictionary *resultDic = @{
                                        HCKBeaconAdditionalInformation:(self.additionalInformation ?: @{}),
                                        HCKBeaconDataInformation:self.dataList,
                                        //对于有附加信息的，lev为2，对于普通不包含附加信息的，lev为1.
                                        HCKBeaconDataStatusLev:(self.additionalInformation ? @"2" : @"1"),
                                        };
            if (self.completeBlock) {
...
- (void)setRespondCount:(NSString *)respondNumber{
    if (!HCKBeaconValidStr(respondNumber)) {
        return;
    }
...
- (void)parseDataSuccess:(NSDictionary *)dataDic{
    ...
    HCKBeaconTaskOperationID operationID = [dataDic[@"operationID"] integerValue];
    if (operationID == HCKBeaconTaskDefaultOperation || operationID != self.operationID) {
        return;
    }
    ...
    NSString *numString = returnData[HCKBeaconCommunicationDataNum];
    if (HCKBeaconValidStr(numString)) {
        //本条数据是总数信息
